...
module ParallelCalabash
  ...
  class AndroidRunner
    ...
    def command_for_test(process_number, base_command, apk_path, cucumber_options, test_files)
      ...
      apk_name = apk_path[apk_path.index('bin\\')+4..apk_path.index('.apk')-1]
      separator = (WINDOWS ? ' & ' : ';')
      exports = env.map { |k, v| WINDOWS ? "(SET \"#{k}=#{v}\")" : "#{k}=#{v};export #{k}" }.join(separator)
      cmd += " --format html --out html_reports/#{device_info}_#{apk_name}_#{Time.now.strftime('%d%b')}_report.html"
